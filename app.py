...
def generate_curriculum_plan(school_level, grade, weekly_hours):
    prompt = f"""
...
weekly_hours = st.number_input("주당 수업 시수", min_value=1, max_value=6, value=4)

if st.button("교육과정 계획 생성"):
    curriculum_plan = generate_curriculum_plan(school_level, grade, weekly_hours)
...
### 내용 영역
{', '.join(content_areas[school_level])}
...
### 디지털 도구
- 기기: {', '.join(digital_tools['기기'])}
- 플랫폼: {', '.join(digital_tools['플랫폼'])}
- 활용 방법: {', '.join(digital_tools['활용 방법'])}
